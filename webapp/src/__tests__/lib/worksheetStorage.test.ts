...
function makeWorksheet(
  ...
  return {
    ...
    problems: [{ operand1: 1, operand2: 1, operation: "addition" }],
    createdAt,
...
describe("worksheetStorage - search, pagination, ratings", () => {
  ...
  it("listWorksheets returns sorted and paginated results", async () => {
    ...
    const res = await listWorksheets(ctx, {
      sortBy: "views",
      limit: 1,
      offset: 0,
    });
    ...
    const page2 = await listWorksheets(ctx, {
      sortBy: "views",
      limit: 1,
      offset: 1,
    });
  ...
  it("searchWorksheets finds by title, subtitle, description and paginates", async () => {
    ...
    const res = await searchWorksheets(ctx, "addition", {
      limit: 10,
      offset: 0,
    });
