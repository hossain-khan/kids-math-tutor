...
describe('aiSafety module', () => {
  ...
  describe('checkContentSafetyWithAI', () => {
    ...
    it('should include all content in AI prompt', async () => {
      ...
      expect(messages[0].content).toContain('Test Title');
      expect(messages[0].content).toContain('Test Subtitle');
    ...
    it('should handle AI response parsing errors gracefully', async () => {
      const env = {
        AI: {
          run: vi
            .fn()
            .mockResolvedValue('invalid json'), // Invalid response
        },
      };
  ...
  describe('fallbackToBadWords', () => {
    it('should return safe result for clean text', () => {
      const result = fallbackToBadWords(
        'Math Practice with Addition Problems',
      );
    ...
    it('should detect profanity in text', () => {
      const result = fallbackToBadWords(
        'This damn math problem is hell',
      );
  ...
  describe('Safety check result structure', () => {
    ...
    it('should return explanation only when content is unsafe', async () => {
      ...
      expect(safeResult.explanation).toBe('Content is appropriate for children');
