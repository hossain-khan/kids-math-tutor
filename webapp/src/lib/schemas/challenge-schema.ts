...
export const ProblemSpecSchema = z.object({
  ...
  message: 'Result cannot be negative (first number must be ≥ second number)',
  path: ['operand1'],
});
...
export const GeneratedChallengeSpecSchema = z.object({
  ...
  subtitle: z.string().max(150, 'Subtitle must be 150 characters or less').optional(),
  operation: MathOperationSchema,
  problemCount: z.number().int().min(1, 'Must create at least 1 problem').max(50, 'Maximum 50 problems allowed'),
...
export const ExplicitChallengeSpecSchema = z.object({
  ...
  title: z.string().min(1, 'Title is required').max(100, 'Title must be 100 characters or less'),
  subtitle: z.string().max(150, 'Subtitle must be 150 characters or less').optional(),
  problems: z.array(ProblemSpecSchema).min(1, 'At least one problem is required').max(50, 'Maximum 50 problems allowed'),
});
